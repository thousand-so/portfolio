/*
* @name 	jQuery Slide - pagination animate
* @version 	1.0.0
* */
(function($) {
    "use strict";
    function mySlide(selector, speed, rollingSpeed, startNumber, fade, auto){
        this.$slide = $(selector);
        this.auto = null; //auto play setInterval
        this.playing = auto;
        this.speed = speed;
        this.rollingSpeed = rollingSpeed;
        this.fade = fade;
        this.start = startNumber;
        this.rerun = null; //next, prev 눌렀을 때 3초간 멈춘 후 자동재생 setTimeout
        this.page = null; //pagination animate setInterval
    ...
    mySlide.prototype = {
        ...
        pageInit : function(){
            for(var i=0; i < this.len; i++){
                var pageStr = "<button type='button' class='page"+(i+1)+"'><span class='line'></span><span class='text'>"+(i+1)+"</span></button>";
        ...
        //페이지네이션
        pagination : function(){
            this.$pageItem.find(".line").attr("style",""); //rolling 일 때, 적용 된 스타일 삭제
            this.$pageItem.removeClass("is-active");
            this.$pageItem.eq(this.target).addClass("is-active");
        },

        paginationRoll : function(){
            this.$pageItem.find(".line").attr("style","");

            var time = this.rollingSpeed / 100 ;
            var $item = this.$pageItem.eq(this.target).find(".line");
            var percent = 0;

            if( this.page != null ) clearInterval(this.page);
            this.page = setInterval(function(){
                percent++;
        ...
        play : function(){
            var me = this;
            me.stop();
            me.paginationRoll(this.target); //rollingSpeed 초후에 시작되므로 지금 보고있는 슬라이드의 bar 갱신
